...
all_words = open("mots.txt", "r").read().splitlines()
...
class hanged_man():
    ...
    def is_valid(self, enter):
        # 65 = 'A', 90 = 'Z'
        if not self.is_word(enter):
            if len(enter) == 1 and ord(enter) >= 65 and ord(enter) <= 90:
                return True
        return False
    ...
    def guess(self, enter):
        if self.is_word(enter):
            ...
            self.mistakes += 1
            return
        if not self.is_valid(enter):
            return print("veuillez entrer une lettre ou un mot valide")
...
def main():
    ...
    while not word.found and word.mistakes < 10:
        ...
        if word.found:
            print(f"Bravo, le resultat etait bien {word.result}")
        print("-"*50)
